...
import { Scheduler, asyncSched, perfProvider, frameProvider } from "./scheduler.js"
...
export class Observable<T> implements qt.Observable<T>, qt.Subscribable<T> {
  ...
  constructor(subscribe?: (this: Observable<T>, x: Subscriber<T>) => qt.Teardown) {
  ...
  if (res) {
    if (qu.isScheduler(res)) sched = res
      ...
      return function (this: any, ...xs: any[]) {
        return (_bindCallback(nodeStyle, cb, sched) as any).apply(this, xs).pipe(qu.mapOneOrManyArgs(res as any))
      }
    }
  }
  if (sched) {
    return function (this: any, ...xs: any[]) {
      return (_bindCallback(nodeStyle, cb) as any).apply(this, xs).pipe(subscribeOn(sched!), observeOn(sched!))
    }
  }
...
function maybeSchedule(sched: qt.Scheduler | undefined, f: () => void, s: Subscription) {
  if (sched) sched.run(s, f)
  else f()
}

export function concat<T extends readonly unknown[]>(...xs: [...qt.InputTuple<T>]): Observable<T[number]>
export function concat<T extends readonly unknown[]>(...xs: [...qt.InputTuple<T>, qt.Scheduler]): Observable<T[number]>
...
export function connectable<T>(x: qt.ObsInput<T>, cfg: ConnectableConfig<T> = DEFAULT_CONFIG): qt.Connectable<T> {
...
export function defer<R extends qt.ObsInput<any>>(f: () => R): Observable<qt.ObsValueOf<R>> {
...
export function fromFetch(x: string | Request, init?: RequestInit): Observable<Response>
export function fromFetch<T>(
  x: string | Request,
  init0: RequestInit & {
    selector?: (x: Response) => qt.ObsInput<T>
...
export function webSocket<T>(x: string | WebSocketSubjectConfig<T>): WebSocketSubject<T> {
...
export function from<T extends qt.ObsInput<any>>(x: T): Observable<qt.ObsValueOf<T>>
export function from<T extends qt.ObsInput<any>>(x: T, sched?: qt.Scheduler): Observable<qt.ObsValueOf<T>>
export function from<T>(x: qt.ObsInput<T>, sched?: qt.Scheduler): Observable<T> {
...
export function fromEvent<T>(x: any, n: string, opts?: EventListenerOptions | ((...xs: any[]) => T), res?: (...xs: any[]) => T): Observable<T> {
  ...
  if (res) {
    return fromEvent<T>(x, n, opts as EventListenerOptions).pipe(qu.mapOneOrManyArgs(res))
  }
  const [add, remove] = isEventTarget(x)
    ? eventTargetMethods.map(m => (h: any) => x[m](n, h, opts as EventListenerOptions))
  ...
  if (!add) {
    if (qu.isArrayLike(x)) {
      return mergeMap((x2: any) => fromEvent(x2, n, opts as EventListenerOptions))(innerFrom(x)) as Observable<T>
    }
  }
...
function isJQueryStyleEventEmitter(x: any): x is JQueryStyleEventEmitter<any, any> {
...
export function generate<R, T>(x: T | GenerateOptions<R, T>, cond?: ConditionFunc<T>, iterate?: IterateFunc<T>, rs?: ResultFunc<T, R> | qt.Scheduler, sched?: qt.Scheduler): Observable<R> {
  ...
  if (arguments.length === 1) {
    ;({ x0, cond, iterate, res = qu.identity as ResultFunc<T, R>, sched } = x as GenerateOptions<R, T>)
  } else {
    x0 = x as T
  ...
  return defer((sched ? () => sched?.runIterable(gen()) : gen) as () => qt.ObsInput<R>)
}

export function iif<T, F>(cond: () => boolean, t: qt.ObsInput<T>, f: qt.ObsInput<F>): Observable<T | F> {
  return defer(() => (cond() ? t : f))
}

export function innerFrom<T extends qt.ObsInput<any>>(x: T): Observable<qt.ObsValueOf<T>>
...
export function interval(period = 0, sched: qt.Scheduler = asyncSched): Observable<number> {
  if (period < 0) period = 0
  return timer(period, period, sched)
}

export function merge<T extends readonly unknown[]>(...xs: [...qt.InputTuple<T>]): Observable<T[number]>
export function merge<T extends readonly unknown[]>(...xs: [...qt.InputTuple<T>, number?]): Observable<T[number]>
export function merge(...xs: (qt.ObsInput<unknown> | number | qt.Scheduler)[]) {
  ...
  return !ys.length ? EMPTY : ys.length === 1 ? innerFrom(ys[0]!) : mergeAll(concurrent)(from(ys, sched))
}
...
export function of<T extends readonly unknown[]>(...xs: T): Observable<qt.ValueFromArray<T>>
...
export function race<T extends readonly unknown[]>(...xs: [...qt.InputTuple<T>]): Observable<T[number]>
export function race<T>(...xs: (qt.ObsInput<T> | qt.ObsInput<T>[])[]): Observable<any> {
  xs = qu.argsOrArgArray(xs)
  return xs.length === 1 ? innerFrom(xs[0] as qt.ObsInput<T>) : new Observable<T>(raceInit(xs as qt.ObsInput<T>[]))
}
...
export function range(start: number, count?: number, sched?: qt.Scheduler): Observable<number> {
  if (count == null) {
    count = start
    start = 0
  }
...
export function zip<T extends readonly unknown[]>(xs: [...qt.InputTuple<T>]): Observable<T>
export function zip<T extends readonly unknown[], R>(xs: [...qt.InputTuple<T>], sel: (...xs: T) => R): Observable<R>
export function zip<T extends readonly unknown[]>(...xs: [...qt.InputTuple<T>]): Observable<T>
export function zip<A extends readonly unknown[], R>(...xs: [...qt.InputTuple<A>, (...xs2: A) => R]): Observable<R>
...
export class WebSocketSubject<T> extends AnonymousSubject<T> {
  ...
  constructor(urlConfigOrSource: string | WebSocketSubjectConfig<T> | Observable<T>, destination?: qt.Observer<T>) {
    super()
  ...
  override lift<R>(x: Operator<T, R>): WebSocketSubject<R> {
    const y = new WebSocketSubject<R>(this._config as WebSocketSubjectConfig<any>, this.dest as any)
    y.op = x
    y.src = this
    return y
  }
    ...
    socket.onopen = (evt: Event) => {
      ...
      this.dest = Subscriber.create<T>(
        ...
        x => {
          ...
          else observer.error(new TypeError(WEBSOCKETSUBJECT_INVALID_ERROR_OBJECT))
          this._resetState()
        },
        () => {
          const { closingObserver: closing } = this._config
