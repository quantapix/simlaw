...
export type PartialObserver<T> = NextObserver<T> | ErrObserver<T> | DoneObserver<T>
...
export interface Observable<T> {
  ...
  subscribe(next?: PartialObserver<T> | ((x: T) => void), error?: (x: any) => void, done?: () => void): Subscription
}
...
export interface Scheduler extends TimestampProvider {
  schedule<T>(work: (this: SchedulerAction<T>, x?: T) => void, delay?: number, x?: T): Subscription
...
export type ObservedValueUnionFromArray<X> = X extends Array<ObsInput<infer T>> ? T : never
...
export type Cons<X, XS extends readonly any[]> = ((x: X, ...xs: XS) => any) extends (...xs: infer T) => any ? T : never
export type Head<XS extends readonly any[]> = ((...xs: XS) => any) extends (x: infer T, ...xs: any[]) => any ? T : never
...
interface ReadableStreamDefaultReaderLike<T> {
  read(): PromiseLike<{ done: false; value: T } | { done: true; value?: undefined }>
  releaseLock(): void
}
