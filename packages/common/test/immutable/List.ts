...
describe("List", () => {
  ...
  it("throw when calling setIn on a non data structure", () => {
    ...
    }).toThrow("Cannot update within non-data-structure value in path [0]: ironMan")
  })
  ...
  it("iterates a dense list", () => {
    ...
    expect(arrayResults).toEqual([undefined, 1, undefined, 3, undefined, 5, undefined, 7, undefined, 9, undefined])
  ...
  check.it("pushes multiple values to the end", { maxSize: 2000 }, [gen.posInt, gen.posInt], (s1, s2) => {
  ...
  check.it("pop removes the highest index, just like array", { maxSize: 2000 }, [gen.posInt], len => {
  ...
  check.it("push adds the next highest index, just like array", { maxSize: 2000 }, [gen.posInt], len => {
    const a: Array<any> = []
    let v = List()
  ...
  check.it("unshifts multiple values to the front", { maxSize: 2000 }, [gen.posInt, gen.posInt], (s1, s2) => {
    const a1 = arrayOfSize(s1)
    const a2 = arrayOfSize(s2)
  ...
  it("discards truncated elements when using slice", () => {
    ...
    expect(v3.toArray()).toEqual(list.slice(0, 3).concat([undefined, undefined, undefined] as any))
  })
