...
describe("flatten", () => {
  ...
  describe("flatMap", () => {
    it("first maps, then shallow flattens", () => {
      ...
      const letters = numbers.flatMap(v => fromJS([String.fromCharCode(v), String.fromCharCode(v).toUpperCase()]))
    ...
    it("maps to sequenceables, not only Sequences.", () => {
      ...
      const letters = numbers.flatMap(v => [String.fromCharCode(v), String.fromCharCode(v).toUpperCase()])
      expect(letters.toJS()).toEqual(["a", "A", "b", "B", "c", "C"])
    })
  })
})
