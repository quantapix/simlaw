import { fromJS, List, Map, merge, mergeDeep, mergeDeepWith, Record, Set } from "immutable"
...
describe("merge", () => {
  ...
  it("merges two maps with a merge function", () => {
    const m1 = Map({ a: 1, b: 2, c: 3 })
    const m2 = Map({ d: 10, b: 20, e: 30 })
    expect(m1.mergeWith((a: any, b: any) => a + b, m2)).toEqual(Map({ a: 1, b: 22, c: 3, d: 10, e: 30 }))
  })
  ...
  it("provides key as the third argument of merge function", () => {
    ...
    expect(m1.mergeWith((a, b, key) => (key !== "id" ? add(a, b) : b), m2)).toEqual(Map({ id: 10, b: 22, c: 3, e: 30 }))
  })
  ...
  it("can overwrite existing maps", () => {
    expect(
      (fromJS({ a: { x: 1, y: 1 }, b: { x: 2, y: 2 } }) as Map<string, any>).merge({
        a: null,
        b: Map({ x: 10 }),
      })
    ).toEqual(fromJS({ a: null, b: { x: 10 } }))
    expect(
      (fromJS({ a: { x: 1, y: 1 }, b: { x: 2, y: 2 } }) as Map<string, any>).mergeDeep({
        a: null,
        b: { x: 10 },
      })
  ...
  it("merges map entries with new values", () => {
    ...
    expect(initial.merge({ b: [2] } as any)).toEqual(Map({ a: List([1]), b: [2] }))
    expect(initial.mergeDeep({ b: [2] } as any)).toEqual(fromJS(Map({ a: List([1]), b: [2] })))
  })
  ...
  for (const [{ type: type1, value: value1 }, { type: type2, value: value2 }] of incompatibleTypes) {
  ...
  const compatibleTypesAndResult = [
    ...
    [record, { type: "Record", value: RecordFactory({ a: 3 }) }, RecordFactory({ a: 3, b: 2 })],
    [list, { type: "List", value: List(["12"]) }, List(["5", "12"])],
    [array, { type: "array", value: ["3"] }, ["9", "3"]],
    [set, { type: "Set", value: Set(["3", "5"]) }, Set(["3", "5"])],
  ] as const

  for (const [{ type: type1, value: value1 }, { type: type2, value: value2 }, result] of compatibleTypesAndResult) {
