import { fromJS, List, Map, removeIn, Seq, Set, setIn, updateIn } from "immutable"

describe("updateIn", () => {
  it("deep edit", () => {
    const m = fromJS({ a: { b: { c: 10 } } }) as Map<string, unknown>
    expect(m.updateIn(["a", "b", "c"], (value: any) => value * 2).toJS()).toEqual({
      a: { b: { c: 20 } },
    })
  })
  ...
  it("deep edit with list as keyPath", () => {
    const m = fromJS({ a: { b: { c: 10 } } }) as Map<string, unknown>
    expect(m.updateIn(fromJS(["a", "b", "c"]), (value: any) => value * 2).toJS()).toEqual({ a: { b: { c: 20 } } })
  })
  ...
  it("deep edit throws without list or array-like", () => {
    ...
    expect(() => Map().updateIn("abc", x => x)).toThrow("Invalid keyPath: expected Ordered Collection or Array: abc")
  })
  ...
  it("deep edit throws if non-editable path", () => {
    ...
    expect(() => deepSeq.updateIn(["key", "foo", "item"], () => "newval")).toThrow(
  ...
  it("deep remove", () => {
    const m = fromJS({ a: { b: { c: 10 } } }) as Map<string, unknown>
    expect(m.updateIn(["a", "b"], (map: any) => map.remove("c")).toJS()).toEqual({
      a: { b: {} },
    })
  })
  ...
  it("deep set", () => {
    const m = fromJS({ a: { b: { c: 10 } } }) as Map<string, unknown>
    expect(m.updateIn(["a", "b"], (map: any) => map.set("d", 20)).toJS()).toEqual({
      a: { b: { c: 10, d: 20 } },
    })
  })
  ...
  it("deep map", () => {
    const m = fromJS({ a: { b: [1, 2, 3] } }) as Map<string, unknown>
    expect(m.updateIn(["a", "b"], (list: any) => list.map(value => value * 10)).toJS()).toEqual({
      a: { b: [10, 20, 30] },
    })
  })
  ...
  it("creates new maps if path contains gaps", () => {
    const m = fromJS({ a: { b: { c: 10 } } }) as Map<string, unknown>
    expect(m.updateIn(["a", "q", "z"], Map(), (map: any) => map.set("d", 20)).toJS()).toEqual({
      a: { b: { c: 10 }, q: { z: { d: 20 } } },
    })
  })
  ...
  it("creates new objects if path contains gaps within raw JS", () => {
    const m = { a: { b: { c: 10 } } }
    expect(updateIn(m, ["a", "b", "z"], Map(), (map: any) => map.set("d", 20))).toEqual({
      a: { b: { c: 10, z: Map({ d: 20 }) } },
    })
  })
  ...
  describe("removeIn", () => {
    it("provides shorthand for updateIn to remove a single value", () => {
      const m = fromJS({ a: { b: { c: "X", d: "Y" } } }) as Map<string, unknown>
  ...
  describe("mergeIn", () => {
    ...
    it("merges into plain JS Object and Array", () => {
      ...
      expect(m.mergeIn(["a", "x"], [4, 5, 6])).toEqual(Map({ a: { x: [1, 2, 3, 4, 5, 6] } }))
    })
  })
  ...
  describe("mergeDeepIn", () => {
    ...
    it("merges deep into plain JS Object and Array", () => {
      const m = Map({ a: { x: [1, 2, 3] } })
      expect(m.mergeDeepIn(["a"], { x: [4, 5, 6] })).toEqual(Map({ a: { x: [1, 2, 3, 4, 5, 6] } }))
    })
  })
})
