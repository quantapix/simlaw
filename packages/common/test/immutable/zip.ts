...
describe("zip", () => {
  ...
  check.it("is always the size of the smaller sequence", [gen.array(gen.posInt).notEmpty()], lengths => {
  ...
  describe("zipAll", () => {
    ...
    check.it("is always the size of the longest sequence", [gen.array(gen.posInt).notEmpty()], lengths => {
      const ranges = lengths.map(l => Range(0, l))
      const first = ranges.shift()
