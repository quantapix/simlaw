...
describe("curry", () => {
  ...
  it("support currying for produceWithPatches", () => {
    const y = produceWithPatches((x, delta) => {
      x.x += delta
    })
...
describe("current", () => {
  ...
  it("won't freeze", () => {
    ...
    const y = produce(base, x => {
      x.x++
      expect(Object.isFrozen(current(x))).toBe(false)
    })
  })
  it("returns original without changes", () => {
    const base = {}
    produce(base, x => {
      expect(original(x)).toBe(base)
      expect(current(x)).toBe(base)
    })
  })
  ...
  it("will find drafts inside objects", () => {
    ...
    produce(base, x => {
      ...
      expect(c.y.nested).not.toBe(x.y.nested)
    })
  })
  it("handles map - 1", () => {
    ...
    produce(base, x => {
      ...
      x.delete("a")
      const c = current(x)
      expect(current(x)).not.toBe(base)
      expect(current(x)).not.toBe(x)
      expect(c).toEqual(new Map())
      const obj = {}
      x.set("b", obj)
      expect(c).toEqual(new Map())
      expect(current(x)).toEqual(new Map([["b", obj]]))
...
describe("auto freeze", () => {
  ...
  describe("the result is always auto-frozen when", () => {
    ...
    it("a nested draft is returned", () => {
      const base: { a: any } = { a: {} }
      const y = produce(base, x => x.a)
      expect(y).toBe(base.a)
      expect(isFrozen(y)).toBeTruthy()
    })
    it("the base state is returned", () => {
      const base = {}
      const y = produce(base, () => base)
      expect(y).toBe(base)
      expect(isFrozen(y)).toBeTruthy()
    })
  ...
  it("will freeze maps", () => {
    ...
    expect(produce(y, x => void x.set("a", 2))).not.toBe(y)
  })
  it("will freeze sets", () => {
    const base = new Set()
    const y = produce(base, _ => {
      base.add(1)
    })
    ...
    expect(produce(y, x => void x.add(2))).not.toBe(y)
  })
...
describe("manual", () => {
  ...
  if (!isProd)
    it("cannot modify after finish", () => {
      ...
      const d = createDraft(base)
      d.a = 2
      expect(finishDraft(d)).toEqual({ a: 2 })
      expect(() => {
        d.a = 3
      }).toThrowErrorMatchingSnapshot()
    })
  it("should support patches drafts", () => {
    const base = { a: 1 }
    const d: any = createDraft(base)
    d.a = 2
    d.b = 3
    const mock = jest.fn()
    const y = finishDraft(d, mock)
    expect(y).not.toBe(base)
  ...
  it("should handle multiple create draft calls", () => {
    ...
    const d = createDraft(base)
    d.a = 2
    const d2: any = createDraft(base)
    d2.b = 3
    const y = finishDraft(d)
    expect(y).not.toBe(base)
    expect(y).toEqual({ a: 2 })
    d2.a = 4
    const y2 = finishDraft(d2)
  ...
  it("combines with produce - 1", () => {
    ...
    const d: any = createDraft(base)
    d.a = 2
    const y = produce(d, (x: any) => {
      x.b = 3
    })
    d.b = 4
    const y2 = finishDraft(d)
    expect(y).toEqual({ a: 2, b: 3 })
  ...
  !global.USES_BUILD &&
    it("should not finish drafts from produce", () => {
      produce({ x: 1 }, x => {
        expect(() => finishDraft(x)).toThrowErrorMatchingSnapshot()
      })
    })
  it("should not finish twice", () => {
    const d = createDraft({ a: 1 })
    d.a++
    finishDraft(d)
    expect(() => finishDraft(d)).toThrowErrorMatchingSnapshot()
  })
})
...
describe("original", () => {
  ...
  it("should return the original from the draft", () => {
    produce(base, x => {
      expect(original(x)).toBe(base)
      expect(original(x.a)).toBe(base.a)
      expect(original(x.b)).toBe(base.b)
    })
    produce(base, x => {
      expect(original(x)).toBe(base)
      expect(original(x.a)).toBe(base.a)
  ...
  it("should throw undefined for new values on the draft", () => {
    produce(base, (x: any) => {
      x.c = {}
      x.d = 3
      expect(() => original(x.c)).toThrowErrorMatchingInlineSnapshot(
        isProd
      ...
      expect(() => original(x.d)).toThrowErrorMatchingInlineSnapshot(
        isProd
...
describe("map issues", () => {
  ...
  it("setNoPatches", () => {
    ...
    const y = produceWithPatches(base, x => {
      x.set.add("abc")
    })
    expect(y).toEqual([
      { set: new Set(["abc"]) },
  ...
  it("mapChangeBug2 ", () => {
    ...
    const base1 = produce(base, _ => {})
    const [y, p, ip] = produceWithPatches(base1, x => {
      const a = x.map.get("a")
      a?.forEach((_, x2) => {
        const m = x.map.get(x2)
        m?.delete("a")
...
describe("xxx", () => {
  ...
  it("yyy", () => {
    ...
    expect(base.stock[DRAFTABLE]).toBeTruthy()
    const y: State = applyPatches(base, errorProducingPatch)
    expect(base.stock.price).toEqual(100)
    expect(y.stock.price).toEqual(200)
    expect(y.stock[DRAFTABLE]).toBeTruthy()
    const y2 = produce(y, x => {
      x.stock.pushPrice(300)
    })
    expect(base.stock.price).toEqual(100)
    expect(y2.stock.price).toEqual(300)
    expect(y2.stock[DRAFTABLE]).toBeTruthy()
    expect(y.stock.price).toEqual(200)
  })
})
