...
export function isMatching<p extends qt.Pattern<any>>(
  pattern: p,
  value: any
): value is qt.MatchedValue<any, Infer<p>>
...
export function isMatching<p extends qt.Pattern<any>>(
  ...
  if (args.length === 1) {
    const [pattern] = args
    return (value: any): value is qt.MatchedValue<any, Infer<p>> =>
      qu.matchPattern(pattern, value, () => {})
  }
  if (args.length === 2) {
...
export const match = <input, output = qt.unset>(
  value: input
): qt.Match<input, output> => new MatchExpression(value, []) as any
...
class MatchExpression<i, o> {
  ...
  otherwise<c>(
    handler: (value: i) => qt.PickReturnValue<o, c>
  ): qt.PickReturnValue<o, c> {
    return new MatchExpression<i, qt.PickReturnValue<o, c>>(
      this.value,
      this.cases.concat([
        {
...
>(pattern: p): qt.OptionalP<input, p> {
  return {
    [qt.matcher]() {
      return {
...
export function array<
  input,
  p extends unknown extends input ? qt.UnknownPattern : qt.Pattern<Elem<input>>
>(pattern: p): qt.ArrayP<input, p> {
  return {
    [qt.matcher]() {
      return {
        match: <I>(value: I | input) => {
          ...
          const matched = value.every(v =>
            qu.matchPattern(pattern, v, selector)
          )
          return { matched, selections }
        },
        getSelectionKeys: () => qu.getSelectionKeys(pattern),
  ...
  return {
    [qt.matcher]: () => ({
      match: value => {
        ...
        const matched = (patterns as qt.UnknownPattern[]).every(p =>
          qu.matchPattern(p, value, selector)
        )
        return { matched, selections }
      },
      getSelectionKeys: () =>
        qu.flatMap(patterns as qt.UnknownPattern[], qu.getSelectionKeys),
      matcherType: "and",
    }),
  }
}
  ...
  return {
    [qt.matcher]: () => ({
      match: <I>(value: I | input) => {
        ...
        qu.flatMap(
          patterns as qt.UnknownPattern[],
          qu.getSelectionKeys
        ).forEach(key => selector(key, undefined))
        const matched = (patterns as qt.UnknownPattern[]).some(p =>
          qu.matchPattern(p, value, selector)
        )
        return { matched, selections }
      },
...
export function not<
  ...
  p extends unknown extends input
    ? qt.UnknownPattern
    : qt.Pattern<input> | undefined
>(pattern: p): qt.NotP<input, p> {
  return {
    [qt.matcher]: () => ({
      match: <I>(value: I | input) => ({
...
export function select<
  ...
  patternOrKey extends
    | string
    | (unknown extends input ? qt.UnknownPattern : qt.Pattern<input>)
>(
  patternOrKey: patternOrKey
): patternOrKey extends string
  ? qt.SelectP<patternOrKey>
  : qt.SelectP<qt.anonymousSelectKey, input, patternOrKey>
export function select<
  input,
  p extends unknown extends input ? qt.UnknownPattern : qt.Pattern<input>,
  k extends string
        ...
        match: value => {
          ...
          return {
            matched:
              pattern === undefined
                ? true
                : qu.matchPattern(pattern, value, selector),
            selections: selections,
          }
        },
        getSelectionKeys: () =>
          [key ?? qt.anonymousSelectKey].concat(
            pattern === undefined ? [] : qu.getSelectionKeys(pattern)
          ),
      }
    },
  }
...
export function instanceOf<T extends AnyConstructor>(
  classConstructor: T
): qt.GuardP<unknown, InstanceType<T>> {
...
export function typed<input>(): {
  ...
  intersection<ps extends [qt.Pattern<input>, ...qt.Pattern<input>[]]>(
    ...patterns: ps
  ): qt.AndP<input, ps>
  union<ps extends [qt.Pattern<input>, ...qt.Pattern<input>[]]>(
    ...patterns: ps
  ): qt.OrP<input, ps>
  not<p extends qt.Pattern<input>>(pattern: p): qt.NotP<input, p>
  when<narrowed extends input = never>(
    predicate: qt.GuardFunction<input, narrowed>
  ): qt.GuardP<input, narrowed>
  select<pattern extends qt.Pattern<input>>(
    pattern: pattern
  ): qt.SelectP<qt.anonymousSelectKey, input, pattern>
  select<p extends qt.Pattern<input>, k extends string>(
    key: k,
    pattern: p
  ): qt.SelectP<k, input, p>
} {
  return {
    array: array as any,
