...
export class Graph<G extends Gdata, N extends Ndata, E extends Edata> {
  createNodes(sel: qt.Sel, shapes = qs.shapes) {
    ...
    const es = sel
      ...
      .data(ns, (n: any) => n)
      .classed("update", true)
    es.enter().append("g").attr("class", "node").style("opacity", 0)
    es.exit().remove()
    sel.selectAll("g.node").each((n: any, i: number, g: any) => {
      ...
      l.attr(
        "transform",
        "translate(" + (nd.pad.left - nd.pad.right) / 2 + "," + (nd.pad.top - nd.pad.bottom) / 2 + ")"
      )
      e = d3.select(g[i])
    ...
    this.applyTransition(s).style("opacity", 0).remove()
    return es
  }
  ...
  enter(sel: qt.Sel) {
    const es = sel.enter().append("g").attr("class", "edgePath").style("opacity", 0)
    es.append("path")
      .attr("class", "path")
      .attr("d", (l: qg.Link<E>) => {
        const ed = this.edge(l)!
  ...
  exit(sel: qt.Sel) {
    const es = sel.exit()
    this.applyTransition(es).style("opacity", 0).remove()
  }
  ...
  createLabels(sel: qt.Sel) {
    let es: qt.Sel = sel
      ...
      .classed("update", true)
    es.exit().remove()
    es.enter().append("g").classed("edgeLabel", true).style("opacity", 0)
...
function addText(s: qt.Sel, d: Data) {
  ...
  for (let i = 0; i < lines.length; i++) {
    es.append("tspan").attr("xml:space", "preserve").attr("dy", "1em").attr("x", "1").text(lines[i])
  }
