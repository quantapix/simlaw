...
export interface Graph<G extends qt.Gdata, N extends qt.Ndata, E extends qt.Edata>
...
export class Graph<G extends qt.Gdata, N extends qt.Ndata, E extends qt.Edata> {
  debugOrdering() {
    ...
    const h = new qt.Graph({ isCompound: true, isMultiple: true }).setData({} as G)
  ...
  postLayout(root: qt.Sel, sel: qt.Sel) {
    ...
    this.edges().forEach(e => {
      ...
      const path = sel.insert("path", ":first-child").classed("edge", true).attr("marker-end", "url(#arrowhead)")
      const line = d3
        .line<qt.Point>()
        .x(p => p.x)
        .y(p => p.y)
    ...
    const walk = (n: string) => {
      ...
      if (cs?.length) {
        ...
        sel
          ...
          .attr("transform", "translate(" + (nd.x - nd.w / 2) + "," + (nd.y - nd.h / 2) + ")")
          .append("rect")
          .attr("width", nd.w)
          .attr("height", nd.h)
      }
...
function appendLabel(sel: qt.Sel, label: any, graphObj: any, marginX: number, marginY: number) {
  ...
  text.attr("transform", "translate(" + -a.width / 2 + "," + -a.height / 2 + ")")
  a = e.node()!.getBBox()
  rect
    .attr("rx", 5)
    .attr("ry", 5)
