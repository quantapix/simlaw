...
const keys = {
  graph: ["acyclicer", "align", "edgesep", "margin", "nodesep", "rankdir", "ranker", "ranksep"],
...
export interface Gdata extends qa.Gdata, qc.Gdata, qn.Gdata, qp.Gdata, qr.Gdata, qt.Area {
  maxRank: number
  margin: qt.Point
}
export interface Ndata extends qa.Ndata, qc.Ndata, qn.Ndata, qp.Ndata, qr.Ndata {
  selfLinks: qg.Link<Edata>[]
  link: qg.Link<Edata>
}
export interface Edata extends qa.Edata, qc.Edata, qn.Edata, qp.Edata, qr.Edata {
  offset: number
}
...
export class Graph<G extends Gdata, N extends Ndata, E extends Edata> {
  ...
  runSteps() {
    this.spreadOut().delSelfLinks().runAcycler().runNest().nonCompound().runRank()
