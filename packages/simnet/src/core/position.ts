...
export class Graph<G extends Gdata, N extends Ndata, E extends Edata> {
  ...
  type2Conflicts(lm: string[][]) {
    const cs = new Conflicts()
    const scan = (ns: string[], s: number, e: number, o0: number, o1: number) => {
      for (let i = s; i < e; i++) {
        const n1 = ns[i]
  ...
  horizontal(lm: string[][], roots: Projs, cols: Projs, reverse = false) {
    ...
    const walk = (pass: (n: string) => void, next: (x: any) => string[] | undefined) => {
...
export function columns(lm: string[][], cs: Conflicts, fn: (n: any) => string[] | undefined) {
...
export function balance(xss: XSS, align?: string) {
  ...
  if (a) {
    return new Map(Array.from(ul, ([n, _]) => [n, xss.get(a)!.get(n)!] as Pair))
  }
