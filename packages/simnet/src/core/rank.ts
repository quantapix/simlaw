...
export class Graph<G extends Gdata, N extends Ndata, E extends Edata> {
  ...
  initRank() {
    ...
    const walk = (n: string) => {
      ...
      if (!done.has(n)) {
        ...
        const rs = this.outLinks(n)?.map(l => walk(l.nodes[1]) - this.edge(l)!.minlen)
  ...
  rankTree() {
    ...
    while (this.tightTree(t) < c) {
      ...
      const delta = t.hasNode(l.nodes[0]) ? this.rankSlack(l) : -this.rankSlack(l)
  ...
  enterLink(t: this, o: qg.Link<E>) {
    ...
    const ls = this.links().filter(l => flip === isDesc(t.node(l.nodes[0])!) && flip !== isDesc(t.node(l.nodes[1])!))
  ...
  normalizeRanks() {
    const min = Math.min(...this.nodes().map(n => this.node(n)?.rank ?? Infinity))
  ...
  delEmptyRanks() {
    const off = Math.min(...this.nodes().map(n => this.node(n)?.rank ?? Infinity))
