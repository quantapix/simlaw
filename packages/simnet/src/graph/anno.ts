...
export class Anno implements qg.Anno {
  ...
  constructor(public type: qt.AnnoT, public nd: qg.Ndata, public ed: qg.Edata, public inbound?: boolean) {
  ...
  position(sel: qt.Sel, d: qg.Ndata, e: qs.Elem) {
    const cx = d.centerX()
    if (this.nd && this.type !== qt.AnnoT.DOTS) this.nd.stylize(sel, e, qt.Class.Anno.NODE)
    if (this.type === qt.AnnoT.SUMMARY) this.w += 10
    sel
      .select("text." + qt.Class.Anno.LABEL)
      .transition()
      .attr("x", cx + this.x + (this.inbound ? -1 : 1) * (this.w / 2 + this.offset))
      .attr("y", d.y + this.y)
    sel
      .select("use.summary")
      .transition()
  ...
  build(sel: qt.Sel, e: qs.Elem) {
    if (this.type === qt.AnnoT.SUMMARY) {
      ...
      s.attr("class", "summary").attr("xlink:href", "#summary-icon").attr("cursor", "pointer")
    } else {
...
export class Annos extends Array<Anno> implements qg.Annos {
  ...
  build(sel: qt.Sel, d: qg.Ndata, e: qs.Elem) {
    ...
    ss.enter()
      ...
      .merge(ss)
      .attr("class", a => qt.Class.Anno.GROUP + toClass(a.type) + qg.toClass(a.nd.type))
      .each(function (a) {
        const s = d3.select(this)
        a.position(s, d, e)
