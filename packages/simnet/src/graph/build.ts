...
export namespace Gdata {
  function buildMeta(gd: qd.Gdata, c: qc.Nclus, meta: qg.Mgraph, core: qg.Cgraph) {
  ...
  export function buildSubhier(this: qd.Gdata, nodeName: string) {
    ...
    bridgeG.links().forEach(l => {
      ...
      const control = !ed.num.regular && cs.control[n1] > qp.GdataPs.maxControlDegree
      const [, annos] = inbound ? [clus.annos.in, d0.annos.in] : [clus.annos.out, d0.annos.out]
      ...
      if (!draw) {
        ...
        annos.push(new qa.Anno(qt.AnnoT.SHORTCUT, n, new qe.Emeta(ed), inbound))
        return
      }
    ...
    ;[true, false].forEach(inbound => {
      ...
      coreG.nodes().forEach(n => {
        ...
        const isTerminal = inbound ? !coreG.preds(n)?.length : !coreG.succs(n)?.length
