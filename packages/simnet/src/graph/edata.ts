...
export class Edata implements qg.Edata {
  ...
  stylize(sel: qt.Sel, _e: qs.Elem, _stylize?: boolean) {
    ...
    const m = this.meta
    sel.select("path." + qt.Class.Edge.LINE).classed("control-dep", !!(m && !m.num.regular))
  }
  ...
  interpolator(
    ...
    if (this.marker.start) {
      ps = adjust(ps, d3.select(shadowRoot!.querySelector("#" + this.marker.start)), true)
    }
    if (this.marker.end) {
      ps = adjust(ps, d3.select(shadowRoot!.querySelector("#" + this.marker.end)), false)
    }
...
function adjust(ps: qt.Point[], marker: qt.Sel, isStart: boolean) {
  const line = d3
    ...
    .x(d => d.x)
    .y(d => d.y)
  const path = d3.select(document.createElementNS("http://www.w3.org/2000/svg", "path")).attr("d", line(ps)!)
  const width = +marker.attr("markerWidth")
  const box = marker.attr("viewBox").split(" ").map(Number)
  const w = box[2] - box[0]
