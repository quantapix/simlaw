...
export function runLayout<G extends qg.Gdata, N extends qg.Ndata, E extends qg.Edata>(
  g: qg.Graph<G, N, E>,
  opts?: qg.Opts
) {
  _.extend(g.data, {
    ...
    switch (nd.type) {
      ...
      case qt.NdataT.LIST:
        if (nd.expanded) {
          ...
          const s = nd.noControls ? qp.PARAMS.nodeSize.list.vertical : qp.PARAMS.nodeSize.list.horizontal
          _.extend(nd, s)
        }
        break
      default:
...
function layoutMeta(m: qg.Nmeta) {
  ...
  m.box.h = ps.labelHeight + Math.max(m.areas.in.h, m.box.h, m.areas.lib.h, m.areas.out.h)
...
function layoutAnno(d: qg.Ndata) {
  ...
  const iy = d3
    ...
    .range([-touch, touch])
  ins.forEach((a, i) => {
    a.points = [new qt.Point(a.x + a.w / 2, a.y), new qt.Point(-d.box.w / 2, ins.length > 1 ? iy(i) : 0)]
  })
  ...
  const oy = d3
    ...
    .range([-touch, touch])
  outs.forEach((a, i) => {
    a.points = [new qt.Point(d.box.w / 2, outs.length > 1 ? oy(i) : 0), new qt.Point(a.x - a.w / 2, a.y)]
  })
...
function updateWidth(nd: qg.Ndata) {
  ...
  nd.width.out = nd.annos.out.length > 0 ? qp.PARAMS.annotations.outboxWidth : 0
  nd.box.w = nd.w
  nd.box.h = nd.h
