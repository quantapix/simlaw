...
export namespace Mgraph {
  export function build(this: qg.Mgraph, h: qh.Hierarchy, names: qt.Dict<string>, ps: qt.HierPs) {
  ...
  export function detect(this: qg.Mgraph, c: Cluster, opts: qg.Opts) {
    ...
    _.each(c, (ns, cluster: string) => {
      ...
      _.each(back, (ids, n) => {
        ...
        const path = n.split("/")
        const parent = path.slice(0, path.length - 1).join("/")
        const s = new qc.Nlist(ls[sn].prefix, ls[sn].suffix, parent, +id, n, opts)
        cs[sn].push(s)
      })
      _.each(cs, (ns2, _) => {
  ...
  export function addList(this: qg.Mgraph, l: qg.Nlist[], ls: qt.Dict<qg.Nlist>, cluster: number, opts: qg.Opts) {
    if (l.length > 1) {
      const name = qu.listName(l[0].prefix, l[0].suffix, l[0].pName, l[0].cluster, l[l.length - 1].cluster)
      const nd = new qc.Nlist(l[0].prefix, l[0].suffix, l[0].pName, cluster, name, opts)
      l.forEach(n => {
