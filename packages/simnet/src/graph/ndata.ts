...
export class Ndata implements qg.Ndata {
  ...
  addButton(sel: qt.Sel, e: qs.Elem) {
    ...
    qs.selectCreate(s, "path", qt.Class.Node.E_BUTTON).attr("d", "M0,-2.2 V2.2 M-2.2,0 H2.2")
    qs.selectCreate(s, "path", qt.Class.Node.C_BUTTON).attr("d", "M-2.2,0 H2.2")
  ...
  build(sel: qt.Sel, _e: qs.Elem, c: string) {
    ...
    switch (this.type) {
      ...
      case qt.NdataT.LIST:
        ...
        qs.selectCreate(s, "use", cs).attr("xlink:href", "#op-series-" + t + "-stamp")
        qs.selectCreate(s, "rect", qt.Class.Node.COLOR).attr("rx", this.r).attr("ry", this.r)
        break
      case qt.NdataT.BRIDGE:
        qs.selectCreate(s, "rect", qt.Class.Node.COLOR).attr("rx", this.r).attr("ry", this.r)
        break
  ...
  stylize(s: qt.Sel, e: qs.Elem, c?: string) {
    ...
    const fill = this.fillFor(e.indexer, e.colorBy.toUpperCase() as qt.ColorBy, exp, e.getGraphSvgRoot())
    n.style("fill", fill)
  ...
  fillFor(tidx = (_: string) => 0, cb: qt.ColorBy, expanded?: boolean, root?: SVGElement) {
    ...
    switch (cb) {
      case qt.ColorBy.STRUCT:
        if (this.type === qt.NdataT.META) {
          ...
          return this.structural ? "#f0e" : (this as any as qg.Nbridge).inbound ? "#0ef" : "#fe0"
      ...
      case qt.ColorBy.DEVICE:
        ...
        return expanded ? cs.EXPANDED : grad("device-" + this.name, this.shade.dev, root)
      case qt.ColorBy.CLUSTER:
        if (!this.shade.clus) return cs.UNKNOWN
        return expanded ? cs.EXPANDED : grad("xla-" + this.name, this.shade.clus, root)
      ...
      case qt.ColorBy.COMPAT:
        ...
        return expanded ? cs.EXPANDED : grad("op-compat-" + this.name, this.shade.comp)
      default:
...
export function strokeFor(fill: string) {
  return fill.startsWith("url") ? qp.MetaColors.GRADIENT : d3.rgb(fill).darker().toString()
}
...
function fontScale(e: qs.Elem) {
  if (!scale) {
    scale = d3
      ...
      .range([e.maxMetaNodeLabelLengthFontSize!, e.minMetaNodeLabelLengthFontSize!])
      .clamp(true)
  }
  return scale
}
