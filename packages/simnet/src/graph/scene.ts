...
export function selectCreate(sel: qt.Sel, t: string, n?: string | string[], prior = false): qt.Sel {
...
export function selectChild(sel: qt.Sel, t: string, n?: string | string[]): qt.Sel {
...
export namespace position {
  ...
  export function button(sel: qt.Sel, nd: qg.Ndata) {
    ...
    sel.selectAll("path").transition().attr("transform", t)
    sel.select("circle").transition().attr("cx", x).attr("cy", y).attr("r", PS.nodeSize.meta.expandButtonRadius)
  }
...
export function fit(s: SVGElement, zoom: SVGGElement, d3zoom: any, cb: () => void) {
...
export function panTo(n: string, s: SVGSVGElement, _zoom: SVGGElement, d3zoom: any) {
  const e = d3.select(s).select(`[data-name="${n}"]`).node() as SVGAElement
  if (!e) {
