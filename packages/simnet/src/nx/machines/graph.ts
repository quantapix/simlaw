...
import { CytoscapeDagreConfig, ParentNode, ProjectEdge, ProjectNode } from "../util-cytoscape"
...
export class GraphService {
  ...
  constructor(private tooltipService: GraphTooltipService, private containerId: string) {
  ...
  handleEvent(event: GraphRenderEvents): {
    ...
    switch (event.type) {
      ...
      case "notifyGraphFilterProjectsByText":
        this.filterProjectsByText(event.search, event.includeProjectsByPath, event.searchDepth)
        break
    ...
    if (this.renderGraph) {
      ...
      if (this.collapseEdges) {
        ...
        elements.edges().forEach(edge => {
          ...
          if (sourceNode.parent().first().id() === targetNode.parent().first().id()) {
            ...
            if (commonAncestors.length > 0) {
              sourceAncestors = sourceNode.ancestors().filter(anc => !commonAncestors.contains(anc))
              targetAncestors = targetNode.ancestors().filter(anc => !commonAncestors.contains(anc))
            } else {
            ...
            } else if (targetAncestors.length > 0 && sourceAncestors.length === 0) {
      ...
      if (environmentConfig.environment === "nx-console") {
        ...
        this.renderGraph.fit(this.renderGraph.elements(), 175).center().resize().panBy({ x: 150, y: 0 })
      } else {
        this.renderGraph.fit(this.renderGraph.elements(), 25).center().resize()
      }

      selectedProjectNames = this.renderGraph.nodes('[type!="dir"]').map(node => node.id())
  ...
  showProjects(selectedProjectNames: string[]) {
    const currentNodes = this.renderGraph?.nodes() ?? this.traversalGraph.collection()
  ...
  hideProject(projectName: string) {
    ...
    const nodesToAdd = currentNodes.difference(nodeToHide).difference(nodeToHide.ancestors())
    const ancestorsToAdd = nodesToAdd.ancestors()
  ...
  focusProject(focusedProjectName: string, searchDepth: number = 1) {
    ...
    const includedProjects = this.includeProjectsByDepth(focusedProject, searchDepth)
  ...
  filterProjectsByText(search: string, includePath: boolean, searchDepth: number = -1) {
    if (search === "") {
      this.transferToRenderGraph(this.traversalGraph.collection())
    } else {
      ...
      if (includePath) {
        filteredProjects = filteredProjects.union(this.includeProjectsByDepth(filteredProjects, searchDepth))
      }
  ...
  traceProjects(start: string, end: string) {
    const dijkstra = this.traversalGraph.elements().dijkstra({ root: `[id = "${start}"]`, directed: true })
    ...
    paths.forEach(path => {
      for (let i = 0; i < path.length; i++) {
        ...
        if (nextIndex < path.length) {
          finalCollection = finalCollection.union(path[i].edgesTo(path[nextIndex]))
        }
      }
    })

    finalCollection.union(finalCollection.ancestors())
    this.transferToRenderGraph(finalCollection.union(finalCollection.ancestors()))
  }

  private transferToRenderGraph(elements: cy.Collection) {
    let currentFocusedProjectName
    if (this.renderGraph) {
      currentFocusedProjectName = this.renderGraph.nodes(".focused").first().id()
  ...
  private includeProjectsByDepth(projects: cy.NodeCollection | cy.NodeSingular, depth: number = -1) {
  ...
  initGraph(
    ...
    this.generateCytoscapeLayout(allProjects, groupByFolder, workspaceLayout, dependencies, affectedProjectIds)
  }
  ...
  private generateCytoscapeLayout(
    ...
    const elements = this.createElements(allProjects, groupByFolder, workspaceLayout, dependencies, affectedProjectIds)
  ...
  private createElements(
    ...
    const parents: Record<string, { id: string; parentId: string; label: string }> = {}

    projects.forEach(project => {
      const workspaceRoot =
        project.type === "app" || project.type === "e2e" ? workspaceLayout.appsDir : workspaceLayout.libsDir
    ...
    const projectElements = projectNodes.map(projectNode => projectNode.getCytoscapeNodeDef(groupByFolder))

    const edgeElements = edgeNodes.map(edgeNode => edgeNode.getCytosacpeNodeDef())

    elements = projectElements.concat(edgeElements)

    if (groupByFolder) {
      const parentElements = Object.keys(parents).map(id => new ParentNode(parents[id]).getCytoscapeNodeDef())
  ...
  listenForProjectNodeHovers(): void {
    this.renderGraph.on("mouseover", event => {
      ...
      this.renderGraph.elements().difference(node.outgoers().union(node.incomers())).not(node).addClass("transparent")
      node.addClass("highlight").outgoers().union(node.incomers()).addClass("highlight")
    })
    ...
    this.renderGraph.on("mouseout", event => {
      ...
      node.removeClass("highlight").outgoers().union(node.incomers()).removeClass("highlight")
    })
  }
