...
export const routeListener: InvokeCallback<DepGraphUIEvents, DepGraphUIEvents> = callback => {
  const history = createBrowserHistory()

  parseSearchParamsToEvents(history.location.search).forEach(event => callback(event))
}
