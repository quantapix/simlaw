...
export function parseParentDirectoriesFromFilePath(path: string, workspaceRoot: string) {
...
export function hasPath(
  ...
  if (maxSearchDepth === -1 || currentSearchDepth <= maxSearchDepth) {
    for (let d of dependencies[node] || []) {
      ...
      if (hasPath(dependencies, target, d.target, visited, currentSearchDepth + 1, maxSearchDepth)) return true
    }
  }
