...
export class ProjectNode {
  ...
  constructor(private project: ProjectGraphProjectNode, private workspaceRoot: string) {}
  ...
  private getData(groupByFolder: boolean): NodeDataDefinition {
    return {
      ...
      parent: groupByFolder && this.project.data.hasOwnProperty("root") ? this.getParentId() : null,
  ...
  public getAncestors(): Ancestor[] {
    ...
    const directories = parseParentDirectoriesFromFilePath(this.project.data.root, this.workspaceRoot)

    return directories.map((directory, index, allDirectories) => {
      const label = [...allDirectories].slice(0, index + 1).join("/")
      const id = `dir-${label}`
      const parentId = index > 0 ? `dir-${[...allDirectories].slice(0, index).join("/")}` : null
      return {
        label,
        id,
        parentId,
