...
export type Op<T, D, R> = (this: T, d: D, i: number, g: T[] | qt.ArrayLike<T>) => R | undefined
...
export class Selection<T extends Target<D>, D, P extends Proxy<Pd>, Pd> {
  ...
  attr(name: string) {
    ...
    return ns.local ? n?.getAttributeNS(ns.space, ns.local) : n?.getAttribute(name)
  }
  ...
  setAttr(name: string, val?: Op<T, D, qt.Scalar> | qt.Scalar, ...args: any) {
    ...
    return this.each(
      val == undefined ? (ns.local ? rNS : r) : typeof val === "function" ? (ns.local ? fNS : f) : ns.local ? cNS : c
    )
  }
  ...
  private bindIdx<D2>(p: P, g: (T | undefined)[], eg: any[], ug: any[], xg: any[], d: D2[]) {
    const dn = d.length
    for (let i = 0; i < dn; ++i) {
      const n = g[i]
      if (n) {
  ...
  private bindKey<D2>(p: P, g: T[], eg: any[], ug: any[], xg: any[], d: D2[], key?: Op<T | P, D | D2, string>) {
  ...
  setData<D2>(val: D2[], key?: Op<T | P, D | D2, string>): Selection<Target<D2>, D2, P, Pd>
  ...
  enter<T2 extends Etarget & Target<D>>() {
    return new Selection<T2, D, P, Pd>(this.enters || this.groups.map(g => new Array(g.length)), this.parents)
  }

  exit<D2>() {
    return new Selection<Target<D2>, D2, P, Pd>(this.exits || this.groups.map(g => new Array(g.length)), this.parents)
  }
  ...
  insert<T2 extends Target<D>>(name: string | Op<T, D, T2>, prior?: string | Op<T, D, T2>, ...args: any) {
    const c = typeof name === "function" ? name : this.creator<T2>(name)
    const s = prior === undefined ? () => null : typeof prior === "function" ? prior : this.selector(prior)
    function i(this: T) {
      return this.insertBefore(c.apply(this, args), s.apply(this, args) || undefined)
    }
  ...
  on(typename: string, value: Op<G, D, void>, capture?: boolean) {
    ...
    function onAdd(tn) {
      ...
      const wrap = filterEvents.hasOwnProperty(tn.type) ? filterContextListener : contextListener
  ...
  selectAll<T2 extends Target<D> & Element>(sel?: Op<T, D, NodeListOf<T2>> | string) {
  ...
  style(name: string) {
    const n = this.node()
    if (n) {
      return n.style?.getPropertyValue(name) || n.defaultView().getComputedStyle(n, null).getPropertyValue(name)
    }
    return undefined
  }

  setStyle(name: string, val?: qt.Scalar, priority?: "important"): this
  setStyle(name: string, val?: Op<T, D, qt.Scalar | undefined> | qt.Scalar, priority?: "important", ...args: any) {
