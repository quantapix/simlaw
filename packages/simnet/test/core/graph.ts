...
describe("graph", () => {
  ...
  describe("filterNodes", () => {
    it("returns identical when unfiltered", () => {
      g.setData("foo").setNode("a", 123).setPath(["a", "b", "c"]).setEdge(["a", "c"], 456)
  ...
  describe("removeNode", () => {
    ...
    it("is idempotent", () => {
      g.setNode("a").delNode("a").delNode("a")
      expect(g.hasNode("a")).toBeFalse
      expect(g.nodeCount).toBe(0)
    })
    it("removes incident edges", () => {
      g.setEdge(["a", "b"]).setEdge(["b", "c"]).delNode("b")
      expect(g.edgeCount).toBe(0)
    })
    it("removes parent relationships", () => {
      g = new qg.Graph({ isCompound: true })
      g.setParent("c", "b").setParent("b", "a").delNode("b")
  ...
  describe("children", () => {
    ...
    it("returns all nodes without parent when parent not set", () => {
      ...
      expect(g.children(undefined)?.sort(qu.sorter)).toEqual(["b", "c", "parent"])
    })
  })
  ...
  describe("edges", () => {
    ...
    it("returns links for edges", () => {
      g.setEdge(["a", "b"])
      g.setEdge(["b", "c"])
      expect(_.sortBy(g.links(), [0, 1])).toEqual([new qg.Link(["a", "b"], g), new qg.Link(["b", "c"], g)])
    })
  })
  ...
  describe("inLinks", () => {
    ...
    it("can return only edges from specified node", () => {
      ...
      expect(_.sortBy(g.inLinks("b", "a"), 2)).toEqual([new qg.Link(["a", "b"]), new qg.Link(["a", "b", "foo"], g)])
    })
  })
  ...
  describe("outLinks", () => {
    ...
    it("can return only edges to specified node", () => {
      ...
      g.setEdge(["z", "a"])
      g.setEdge(["z", "b"])
      expect(_.sortBy(g.outLinks("a", "b"), 1)).toEqual([new qg.Link(["a", "b"], g), new qg.Link(["a", "b", "foo"], g)])
  ...
  describe("nodeLinks", () => {
    ...
    it("returns all edges that node points at", () => {
      ...
      expect(_.sortBy(g.nodeLinks("b"), [0, 1])).toEqual([new qg.Link(["a", "b"], g), new qg.Link(["b", "c"], g)])
