...
describe("json", () => {
  ...
  it("preserves nodes", () => {
    ...
    expect(rw(new Graph({}).setNode("a", { foo: "bar" } as Ndata)).node("a")).toEqual({
      name: "a",
      foo: "bar",
    } as Ndata)
  })
  it("preserves edges", () => {
    ...
    expect(rw(new Graph({}).setEdge(["a", "b"], { foo: "bar" } as Edata)).edge(["a", "b"])).toEqual({
      name: "a:b",
      foo: "bar",
    } as Edata)
  })
  ...
  it("preserves parent relationships", () => {
    expect(rw(new Graph({ isCompound: true }).setNode("a")).parent("a")).toBeUndefined
    expect(rw(new Graph({ isCompound: true }).setParent("a", "parent")).parent("a")).toBe("parent")
  })
})
