...
describe("util", () => {
  ...
  describe("normalizeRanks", () => {
    ...
    it("works for negative ranks", () => {
      g = new Graph({}).setNode("a", { rank: -3 } as qr.Ndata).setNode("b", { rank: -2 } as qr.Ndata)
      g.normalizeRanks()
      expect(g.node("a")!.rank).toBe(0)
...
describe("rankSimplex", () => {
  ...
  beforeEach(() => {
    g = new Graph({ isMultiple: true })
      .setDefNode(() => ({} as qr.Ndata))
      .setDefEdge(() => ({ minlen: 1, weight: 1 } as qr.Edata))
    t = new Graph({ isDirected: false }).setDefNode(() => ({} as qr.Ndata)).setDefEdge(() => ({} as qr.Edata))
    gg = new Graph({})
      .setDefNode(() => ({} as qr.Ndata))
      .setDefEdge(() => ({ minlen: 1, weight: 1 } as qr.Edata))
      .setPath(["a", "b", "c", "d", "h"])
  ...
  describe("initLowLims", () => {
    it("assigns low, lim, and parent for each node in tree", () => {
      ...
      const d = g.node("d")!
      const e = g.node("e")!
      expect(_.sortBy(g.nodes().map(n => g.node(n)!.lim))).toEqual(_.range(1, 6))
  ...
  describe("cutValue", () => {
    ...
    it("works for 4-node tree with gc -> c -> p -> o, with o -> c", () => {
      g.setEdge(["o", "c"], { weight: 7 } as qr.Edata).setPath(["gc", "c", "p", "o"])
      t.setEdge(["gc", "c"], { cutval: 3 } as qr.Edata).setPath(["c", "p", "o"])
      t.initLowLims("p")
      expect(g.cutValue(t, "c")).toBe(-4)
    })
    it("works for 4-node tree with gc -> c -> p -> o, with o <- c", () => {
      g.setEdge(["c", "o"], { weight: 7 } as qr.Edata).setPath(["gc", "c", "p", "o"])
      t.setEdge(["gc", "c"], { cutval: 3 } as qr.Edata).setPath(["c", "p", "o"])
      t.initLowLims("p")
      expect(g.cutValue(t, "c")).toBe(10)
    })
    it("works for 4-node tree with o -> gc -> c -> p, with o -> c", () => {
      g.setEdge(["o", "c"], { weight: 7 } as qr.Edata).setPath(["o", "gc", "c", "p"])
      t.setEdge(["o", "gc"])
    ...
    it("works for 4-node tree with o -> gc -> c -> p, with o <- c", () => {
      g.setEdge(["c", "o"], { weight: 7 } as qr.Edata).setPath(["o", "gc", "c", "p"])
      t.setEdge(["o", "gc"])
        .setEdge(["gc", "c"], { cutval: 3 } as qr.Edata)
        .setEdge(["c", "p"])
      t.initLowLims("p")
