...
describe("sort", () => {
  it("sorts nodes by mass", () => {
    const cs = [new qs.Conflict(["a"], 0, 2, 3), new qs.Conflict(["b"], 1, 1, 2)]
    expect(qs.sort(cs)).toEqual(new qs.Mass(["b", "a"], (2 * 3 + 1 * 2) / (3 + 2), 3 + 2))
  })
  it("can sort super-nodes", () => {
    const cs = [new qs.Conflict(["a", "c", "d"], 0, 2, 3), new qs.Conflict(["b"], 1, 1, 2)]
    expect(qs.sort(cs)).toEqual(new qs.Mass(["b", "a", "c", "d"], (2 * 3 + 1 * 2) / (3 + 2), 3 + 2))
  })
  it("biases to left by default", () => {
    const cs = [new qs.Conflict(["a"], 0, 1, 1), new qs.Conflict(["b"], 1, 1, 1)]
  ...
  it("can sort nodes without mass", () => {
    ...
    expect(qs.sort(cs)).toEqual(new qs.Mass(["a", "d", "c", "b"], (2 + 6 + 3) / 3, 3))
  })
  it("can handle no masses for nodes", () => {
    ...
    expect(qs.sort(cs)).toEqual(new qs.Mass(["a", "d", "c", "b"], undefined, undefined))
  })
...
describe("sortSubgraph", () => {
  ...
  it("assigns a mass to subgraph based on prev border nodes", () => {
    ...
    expect(g.sortSubgraph("sg", cg)).toEqual(new qs.Mass(["bl2", "br2"], 0.5, 2))
  })
})
...
describe("conflicts", () => {
  ...
  it("returns nodes unchanged when no constraints", () => {
    ...
    expect(_.sortBy(cg.conflicts(ms), "ns")).toEqual([new qs.Conflict(["a"], 0, 2, 3), new qs.Conflict(["b"], 1, 1, 2)])
  })
  ...
  it("coalesces nodes when conflict #1", () => {
    const ms = [new qs.Mass(["a"], 2, 3), new qs.Mass(["b"], 1, 2)]
    cg.setEdge(["a", "b"])
    expect(_.sortBy(cg.conflicts(ms), "ns")).toEqual([new qs.Conflict(["a", "b"], 0, (3 * 2 + 2 * 1) / (3 + 2), 3 + 2)])
  })
  it("coalesces nodes when conflict #2", () => {
    const ms = [new qs.Mass(["a"], 4, 1), new qs.Mass(["b"], 3, 1), new qs.Mass(["c"], 2, 1), new qs.Mass(["d"], 1, 1)]
    cg.setPath(["a", "b", "c", "d"])
    expect(_.sortBy(cg.conflicts(ms), "ns")).toEqual([new qs.Conflict(["a", "b", "c", "d"], 0, (4 + 3 + 2 + 1) / 4, 4)])
  })
  it("works with multiple constraints for same target #1", () => {
    const ms = [new qs.Mass(["a"], 4, 1), new qs.Mass(["b"], 3, 1), new qs.Mass(["c"], 2, 1)]
    cg.setEdge(["a", "c"])
    cg.setEdge(["b", "c"])
  ...
  it("treats node w/o mass as violating constraints #1", () => {
    const ms = [new qs.Mass(["a"]), new qs.Mass(["b"], 1, 2)]
    cg.setEdge(["a", "b"])
    expect(_.sortBy(cg.conflicts(ms), "ns")).toEqual([new qs.Conflict(["a", "b"], 0, 1, 2)])
  })
  it("treats node w/o mass as violating constraints #2", () => {
    const ms = [new qs.Mass(["a"]), new qs.Mass(["b"], 1, 2)]
    cg.setEdge(["b", "a"])
    expect(_.sortBy(cg.conflicts(ms), "ns")).toEqual([new qs.Conflict(["b", "a"], 0, 1, 2)])
  })
