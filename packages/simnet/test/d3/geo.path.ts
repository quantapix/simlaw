...
function testArea(projection, object) {
  return geoPath().projection(projection).area(object)
}
...
function testBounds(projection, object) {
  return geoPath().projection(projection).bounds(object)
}
...
function testCentroid(projection, object) {
  return geoPath().projection(projection).centroid(object)
}
...
it("geoPath.centroid(…) of a geometry collection with a single point", () => {
  assert.deepStrictEqual(
    testCentroid(equirectangular, { type: "GeometryCollection", geometries: [{ type: "Point", coordinates: [0, 0] }] }),
    [480, 250]
  )
})
...
function testPath(projection, object) {
  ...
  geoPath().projection(projection).context(context)(object)
...
it("geoPath(LineString) then geoPath(Point) does not treat the point as part of a line", () => {
  const context = testContext(),
    path = geoPath().projection(equirectangular).context(context)
  path({
    type: "LineString",
    coordinates: [
      [-63, 18],
...
function testPath(projection, object) {
  return geoPath().projection(projection)(object)
}
...
it("geoPath.pointRadius(radius)(Point) renders a point of the given radius", () => {
  assertPathEqual(
    geoPath().projection(equirectangular).pointRadius(10)({
      type: "Point",
      coordinates: [-63, 18],
    }),
...
export function testContext() {
  ...
  return {
    arc(x, y, r) {
      buffer.push({ type: "arc", x: Math.round(x), y: Math.round(y), r: r })
    },
    moveTo(x, y) {
      buffer.push({ type: "moveTo", x: Math.round(x), y: Math.round(y) })
    },
    lineTo(x, y) {
      buffer.push({ type: "lineTo", x: Math.round(x), y: Math.round(y) })
    },
    closePath() {
      buffer.push({ type: "closePath" })
    },
    result() {
      let result = buffer
      buffer = []
