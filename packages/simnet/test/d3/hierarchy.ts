...
it("stratify(data) returns the root node", () => {
  const s = stratify()
  const root = s([{ id: "a" }, { id: "aa", parentId: "a" }, { id: "ab", parentId: "a" }, { id: "aaa", parentId: "aa" }])
...
it("stratify(data) does not require the data to be in topological order", () => {
  const s = stratify()
  const root = s([{ id: "aaa", parentId: "aa" }, { id: "aa", parentId: "a" }, { id: "ab", parentId: "a" }, { id: "a" }])
  assert.deepStrictEqual(noparent(root), {
    id: "a",
    depth: 0,
    height: 2,
...
it("stratify(data) preserves the input order of siblings", () => {
  const s = stratify()
  const root = s([{ id: "aaa", parentId: "aa" }, { id: "ab", parentId: "a" }, { id: "aa", parentId: "a" }, { id: "a" }])
  assert.deepStrictEqual(noparent(root), {
    id: "a",
    depth: 0,
    height: 2,
...
it("stratify(data) accepts an iterable", () => {
  const s = stratify()
  const root = s(
    new Set([{ id: "aaa", parentId: "aa" }, { id: "ab", parentId: "a" }, { id: "aa", parentId: "a" }, { id: "a" }])
  )
  assert.deepStrictEqual(noparent(root), {
    id: "a",
    depth: 0,
...
it("stratify(data) throws an error if multiple parents have the same id", () => {
  const s = stratify()
  assert.throws(() => {
    s([{ id: "a" }, { id: "b", parentId: "a" }, { id: "b", parentId: "a" }, { id: "c", parentId: "b" }])
  }, /\bambiguous\b/)
})
...
it("stratify(data) allows the id to be undefined for leaf nodes", () => {
  const s = stratify()
  const root = s([{ id: "a" }, { parentId: "a" }, { parentId: "a" }])
  assert.deepStrictEqual(noparent(root), {
    id: "a",
    depth: 0,
    height: 1,
...
it("stratify.parentId(id) observes the specified parent id function", () => {
  ...
  const root = s([{ id: "a" }, { id: "aa", foo: "a" }, { id: "ab", foo: "a" }, { id: "aaa", foo: "aa" }])
...
it("stratify.path(path) returns the root node", () => {
  const root = stratify().path(d => d.path)([{ path: "/" }, { path: "/aa" }, { path: "/ab" }, { path: "/aa/aaa" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) correctly handles single-character folders", () => {
  const root = stratify().path(d => d.path)([{ path: "/" }, { path: "/d" }, { path: "/d/123" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) correctly handles empty folders", () => {
  const root = stratify().path(d => d.path)([{ path: "/" }, { path: "//" }, { path: "///" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) correctly handles imputed single-character folders", () => {
  const root = stratify().path(d => d.path)([{ path: "/" }, { path: "/d/123" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) imputes internal nodes", () => {
  const root = stratify().path(d => d.path)([{ path: "/aa/aaa" }, { path: "/ab" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) does not allow duplicate internal paths", () => {
  assert.throws(() => {
    stratify().path(d => d.path)([{ path: "/aa" }, { path: "/aa" }, { path: "/aa/aaa" }, { path: "/aa/aaa" }])
  }, /ambiguous/)
})
...
it("stratify.path(path) implicitly trims trailing slashes", () => {
  const root = stratify().path(d => d.path)([{ path: "/aa/" }, { path: "/ab/" }, { path: "/aa/aaa/" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) does not trim trailing slashes preceded by a slash", () => {
  const root = stratify().path(d => d.path)([{ path: "/aa//" }, { path: "/b" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) does not require the data to be in topological order", () => {
  const root = stratify().path(d => d.path)([{ path: "/aa/aaa" }, { path: "/aa" }, { path: "/ab" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) preserves the input order of siblings", () => {
  const root = stratify().path(d => d.path)([{ path: "/ab" }, { path: "/aa" }, { path: "/aa/aaa" }])
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
...
it("stratify.path(path) accepts an iterable", () => {
  const root = stratify().path(d => d.path)(new Set([{ path: "/ab" }, { path: "/aa" }, { path: "/aa/aaa" }]))
  assert(root instanceof hierarchy)
  assert.deepStrictEqual(noparent(root), {
    id: "/",
    depth: 0,
