...
it("selection.interrupt() dispatches an interrupt event to the started transition on the selected elements", async () => {
  ...
  let interrupts = 0
  const s = select(root)
  const t = s.transition().on("interrupt", () => {
    ++interrupts
  })
...
it("selection.interrupt() prevents a starting transition from initializing tweens", async () => {
  ...
  let tweens = 0
  const s = select(root)
  const t = s.transition().tween("tween", () => {
    ++tweens
  })
  const schedule = root.__transition[t._id]
  await new Promise(resolve =>
...
it(
  "selection.transition(transition) throws an error if the specified transition is not found",
