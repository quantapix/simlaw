...
describe("graph-client", () => {
  before(() => {
    cy.intercept("/assets/graphs/nx.json", { fixture: "nx-examples.json" }).as("getGraph")
    cy.visit("/")
  ...
  describe("text filter", () => {
    ...
    it("should clear selection on reset", () => {
      ...
      getCheckedProjectItems().should(
        "have.length",
        nxExamplesJson.projects.filter(project => project.name.includes("cart")).length
      )
  ...
  describe("select all button", () => {
    it("should check all project items", () => {
      ...
      getCheckedProjectItems().should("have.length", nxExamplesJson.projects.length)
    })
  })
  ...
  describe("deselect all button", () => {
    it("should uncheck all project items", () => {
      getDeselectAllButton().click()
      getUncheckedProjectItems().should("have.length", nxExamplesJson.projects.length)
      getSelectProjectsMessage().should("be.visible")
    })
  })
  ...
  describe("show affected button", () => {
    ...
    it("should check all affected project items", () => {
      ...
      getCheckedProjectItems().should("have.length", affectedJson.affected.length)
  ...
  describe("selecting projects", () => {
    ...
    it("should deselect a project by clicking on the project name again", () => {
      ...
      cy.get('[data-project="cart"][data-active="true"]').should("exist").click({
        force: true,
      })
  ...
  describe("focusing projects in sidebar", () => {
    it("should select appropriate projects", () => {
      ...
      const dependents = Object.keys(nxExamplesJson.dependencies).filter(key =>
        nxExamplesJson.dependencies[key].map(dependencies => dependencies.target).includes("cart")
      )
      getCheckedProjectItems().should("have.length", ["cart", ...dependencies, ...dependents].length)
    })
  })
...
describe("loading graph client with url params", () => {
  beforeEach(() => {
    cy.intercept("/assets/graphs/*", { fixture: "nx-examples.json" }).as("getGraph")
  })
