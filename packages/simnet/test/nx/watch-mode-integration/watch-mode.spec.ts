...
describe("graph-client in watch mode", () => {
  ...
  it("should not re-add new libs if they were un-selected", () => {
    cy.tick(5000)
    cy.get('[data-project*="3"][data-active="true"]').should("exist").click({ force: true })
...
function checkSelectedProjects(expectedNumberOfProjects: number, excludedProjects: string[]) {
