...
tape("stratify(data) returns the root node", function (test) {
  var s = d3_hierarchy.stratify(),
    root = s([{ id: "a" }, { id: "aa", parentId: "a" }, { id: "ab", parentId: "a" }, { id: "aaa", parentId: "aa" }])
...
tape("stratify(data) does not require the data to be in topological order", function (test) {
  var s = d3_hierarchy.stratify(),
    root = s([{ id: "aaa", parentId: "aa" }, { id: "aa", parentId: "a" }, { id: "ab", parentId: "a" }, { id: "a" }])
  test.deepEqual(noparent(root), {
    id: "a",
    depth: 0,
    height: 2,
...
tape("stratify(data) preserves the input order of siblings", function (test) {
  var s = d3_hierarchy.stratify(),
    root = s([{ id: "aaa", parentId: "aa" }, { id: "ab", parentId: "a" }, { id: "aa", parentId: "a" }, { id: "a" }])
  test.deepEqual(noparent(root), {
    id: "a",
    depth: 0,
    height: 2,
...
tape("stratify.id(id) observes the specified id function", function (test) {
  var foo = function (d) {
    ...
    root = s([{ foo: "a" }, { foo: "aa", parentId: "a" }, { foo: "ab", parentId: "a" }, { foo: "aaa", parentId: "aa" }])
  test.equal(s.id(), foo)
  test.deepEqual(noparent(root), {
    id: "a",
    depth: 0,
...
tape("stratify.parentId(id) observes the specified parent id function", function (test) {
  var foo = function (d) {
    ...
    root = s([{ id: "a" }, { id: "aa", foo: "a" }, { id: "ab", foo: "a" }, { id: "aaa", foo: "aa" }])
...
while (true) {
  ...
  ++n
  var circles = new Array(20).fill().map(() => ({ r: r(), x: x(), y: y() })),
    circles2,
...
tape("pack(flare) produces the expected result", test("test/data/flare.csv", "test/data/flare-pack.json"))

function test(input, expected) {
  return function (test) {
    d3_queue.queue().defer(fs.readFile, input, "utf8").defer(fs.readFile, expected, "utf8").await(ready)
...
tape(
  ...
  test("test/data/flare.csv", "test/data/flare-phi.json", d3_hierarchy.treemapSquarify)
)

tape(
  "treemap(flare) produces the expected result with a squarified ratio of 1",
  test("test/data/flare.csv", "test/data/flare-one.json", d3_hierarchy.treemapSquarify.ratio(1))
)
...
function test(input, expected, tile) {
  return function (test) {
    ...
    function ready(error, inputText, expectedText) {
      ...
      var treemap = d3_hierarchy.treemap().tile(tile).size([960, 500])
